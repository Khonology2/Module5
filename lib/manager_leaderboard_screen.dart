...
class _ManagerLeaderboardScreenState extends State<ManagerLeaderboardScreen> {
  ...
  Widget build(BuildContext context) {
    final content = _manager == null
        ...
        : StreamBuilder<List<EmployeeData>>(
            stream: ManagerRealtimeService.getTeamDataStream(
              department: _manager!.department.isEmpty ? null : _manager!.department,
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Failed to load leaderboard', style: const TextStyle(color: AppColors.textSecondary)),
                      const SizedBox(height: 4),
                      Text('${snapshot.error}', style: const TextStyle(color: AppColors.textMuted, fontSize: 12)),
                    ],
                  ),
                );
              }
                ...
                return Center(
                  child: Column(
                    ...
                    children: const [
                      ...
                      SizedBox(height: 8),
                      Text('No opted-in employees found', style: TextStyle(color: AppColors.textSecondary)),
                      SizedBox(height: 4),
                      Text('Ask your team to enable Leaderboard Participation', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
                    ],
                  ),
                );
              }
    ...
    return Scaffold(
      ...
      body: FocusTraversalGroup(child: content),
    );
  }
  ...
  Widget _buildHeaderWithFilters() {
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: selected ? AppColors.activeColor : AppColors.borderColor),
          ),
          child: Text(
            label,
            style: TextStyle(
    ...
    return Row(
      children: [
        const Expanded(
          child: Text('Top Team Performers', style: TextStyle(color: AppColors.textPrimary, fontSize: 18, fontWeight: FontWeight.bold)),
        ),
        ...
        child: Column(
          ...
          children: [
            Container(
              height: height,
              decoration: BoxDecoration(
                gradient: LinearGradient(colors: [color.withValues(alpha: 0.8), color.withValues(alpha: 0.4)], begin: Alignment.topCenter, end: Alignment.bottomCenter),
                ...
                child: Column(
                  ...
                  children: [
                    CircleAvatar(
                      ...
                      child: Text(e.profile.displayName.isNotEmpty ? e.profile.displayName[0].toUpperCase() : '?', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                    ),
                    const SizedBox(height: 4),
                    Text(e.profile.displayName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textPrimary, fontSize: 12)),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text('${e.totalPoints} pts', style: const TextStyle(color: AppColors.textSecondary, fontSize: 11)),
                      ],
                    ),
                  ],
                ),
        ...
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(e.profile.displayName, style: const TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w600)),
              const SizedBox(height: 2),
              Text(e.profile.department, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
            ]),
          ),
          Text(rightText, style: const TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
