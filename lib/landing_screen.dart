...
class _PersonalDevelopmentHubScreenState
  ...
  Future<void> _checkTokenAndAutoLogin({String? manualToken}) async {
    try {
      ...
      final token = manualToken ?? TokenAuthService.extractTokenFromUri().token;
      ...
      if (token == null || token.isEmpty) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Cant login right now, please try to login again from KhonoBuzz.',
              ),
      ...
      final firebaseTokenRaw = validationResponse.firebaseToken;
      final userId = validationResponse.userId;
      final email = validationResponse.email;
      ...
      try {
        ...
        if (userCredential.user != null) {
          final effectiveUserId = userCredential.user!.uid;
          String internalRole;
          ...
          unawaited(
            BackendAuthService.instance.callAuthCallback(
              AuthCallbackPayload(
                userId: effectiveUserId.isNotEmpty ? effectiveUserId : userId,
                email: email,
                role: pdhRole,
                authenticated: true,
              ),
        ...
        if (mounted && e is FirebaseAuthException) {
          if (e.code == 'custom-token-mismatch') {
            ScaffoldMessenger.of(context).showSnackBar(
              ...
              (e.message != null &&
                  e.message!.toLowerCase().contains('api-key-not-valid'))) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'Firebase API key rejected. In Google Cloud (pdh-v2): '
                  'enable Identity Toolkit API and add your site URL to Browser key HTTP referrers.',
                ),
